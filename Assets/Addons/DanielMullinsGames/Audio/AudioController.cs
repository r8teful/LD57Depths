...
public class AudioController : Singleton<AudioController> {
        ...
        AudioParams.Repetition repetition = null, AudioParams.Randomization randomization = null, AudioParams.Distortion distortion = null, bool looping = false)
    {
        ...
        if (source != null)
        {
            ...
            if (distortion != null)
            {
                if (distortion.muffled && distortion.cutoff > 0)
                {
                    MuffleSource(source,distortion.cutoff);
                } 
                else if (distortion.muffled)
                {
                    MuffleSource(source);
                }
                if(distortion.reverb) 
                {
                    ReverbSource(source,distortion.reverbPreset);
                }
            }
        }
    ...
    private void ReverbSource(AudioSource source, AudioReverbPreset preset) {
