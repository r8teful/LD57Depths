...
public class FileDataHandler {
    ...
    private readonly string encryptionCodeWord = "R1kL0v3s1S0ck3t";
    ...
    public MasterGameData Load(string profileId, bool allowRestoreFromBackup = true) {
        ...
        MasterGameData loadedData = null;
        if (File.Exists(fullPath)) {
            try {
                Debug.Log("Loading game at path:" + fullPath);
                ...
                loadedData = JsonUtility.FromJson<MasterGameData>(dataToLoad);
            } catch (Exception e) {
    ...
    public void Save(MasterGameData data, string profileId) {
        // base case - if the profileId is null, return right away
        if (profileId == null) {
            return;
        }
        ...
        try {
            ...
            MasterGameData verifiedGameData = Load(profileId);
    ...
    public Dictionary<string, MasterGameData> LoadAllProfiles() {
        Dictionary<string, MasterGameData> profileDictionary = new Dictionary<string, MasterGameData>();
        ...
        foreach (DirectoryInfo dirInfo in dirInfos) {
            ...
            MasterGameData profileData = Load(profileId);
    ...
    public string GetMostRecentlyUpdatedProfileId() {
        ...
        Dictionary<string, MasterGameData> profilesGameData = LoadAllProfiles();
        foreach (KeyValuePair<string, MasterGameData> pair in profilesGameData) {
            string profileId = pair.Key;
            MasterGameData gameData = pair.Value;
